...
**Automation Testing** هو تقنية في اختبار البرمجيات تستخدم أدوات وسكربتات متخصصة لتنفيذ حالات الاختبار تلقائيًا دون تدخل بشري.  
...
1. **Unit Testing** — اختبار الوحدات للتحقق من وظائف الكود الصغيرة بشكل منفصل.  
2. **Integration Testing** — فحص تكامل الأنظمة والخدمات معًا.  
3. **Regression Testing** — التأكد أن التعديلات الجديدة لم تكسر وظائف سابقة.  
4. **Performance Testing** — اختبار الأداء تحت الضغط وقياس سرعة الاستجابة.  
5. **Security Testing** — البحث عن الثغرات الأمنية ونقاط الضعف.  
6. **API Testing** — اختبار الخدمات الخلفية بدون واجهة المستخدم.  
7. **UI/GUI Testing** — اختبار واجهة المستخدم بصريًا وتفاعليًا.  
8. **Smoke Testing** — فحص سريع للتأكد من استقرار النسخة قبل الاختبار الكامل.  
9. **Acceptance Testing** — اختبار القبول للتأكد من تلبية متطلبات العمل.  
10. **Data-Driven Testing** — تشغيل نفس السكربت بعدة مدخلات مختلفة.
...
1. **Linear / Record & Playback Framework** — تسجّل الخطوات وتعيد تشغيلها تلقائيًا.  
2. **Modular Framework** — تقسيم التطبيق إلى وحدات مستقلة (مثل LEGO).  
3. **Data-Driven Framework** — فصل البيانات عن منطق الاختبار.  
4. **Keyword-Driven Framework** — كتابة الاختبارات بكلمات مفتاحية مفهومة لغير المبرمجين.  
5. **Hybrid Framework** — دمج أكثر من نوع للاستفادة من جميع المزايا.  
6. **BDD (Behavior-Driven Development)** — كتابة الاختبارات بلغة بشرية مثل "Given / When / Then".
...
1. **Selenium** — لاختبار تطبيقات الويب متعددة المتصفحات.  
2. **Appium** — لاختبار تطبيقات الجوال على Android وiOS.  
3. **Cypress** — لاختبارات واجهات JavaScript الحديثة.  
4. **Playwright** — من Microsoft، لاختبار عبر المتصفحات.  
5. **TestNG** — إطار متقدم لجافا لدعم التكامل والتقارير.  
6. **JMeter** — من Apache لاختبار الأداء والضغط.  
7. **Jenkins** — خادم أتمتة لتكامل واختبار مستمر.  
8. **Cucumber** — لأتمتة اختبار BDD بلغة بشرية (Gherkin).
...
4. **Shift-Left Testing** — تنفيذ الاختبارات مبكرًا.  
